...
object DTRuleMiner {
  ...
  def main(args: Array[String]) = {
    ...
    var rulesWithId = new ListBuffer[DataFrame]()
    ...
    val x = rulesWithId.reduce(_ union _).cache
    println("Number of Rules " + x.count())

    val rulesWithBody = x.withColumn("operatorIds", explode(col("antecedant")))
      .join(operator2Id, "operatorIds")
      .withColumnRenamed("operator", "head")
      .drop("operatorIds")

    val rulesWithNegation = x.withColumn("operatorIds", explode(col("negation")))
      .join(operator2Id, "operatorIds")
